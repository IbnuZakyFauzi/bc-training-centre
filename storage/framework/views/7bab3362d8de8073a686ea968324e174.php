...
<section class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
    ...
    <div class="mx-5 mt-5 rounded-xl border border-slate-300 overflow-hidden text-[11px]">
        <div class="grid grid-cols-1 md:grid-cols-2">
            ...
            <div class="divide-y divide-slate-200"><div class="grid grid-cols-[130px_1fr]"><span class="p-2 font-bold bg-slate-50">PERUSAHAAN</span><span class="p-2"><?php echo e(data_get($payload, 'meta.company', 'PT BERAU COAL / PT MTL')); ?></span></div><div class="grid grid-cols-[130px_1fr]"><span class="p-2 font-bold bg-slate-50">TIPE ALAT</span><span class="p-2"><?php echo e($logbook->equipmentCategory->name ?? '-'); ?> (<?php echo e($categoryCode); ?>)</span></div><div class="grid grid-cols-[130px_1fr]"><span class="p-2 font-bold bg-slate-50">NO ALAT</span><span class="p-2"><?php echo e($logbook->unit_code); ?></span></div><div class="grid grid-cols-[130px_1fr]"><span class="p-2 font-bold bg-slate-50">HM / KM</span><span class="p-2"><?php echo e(number_format($logbook->hm_start, 1)); ?> → <?php echo e(number_format($logbook->hm_end, 1)); ?></span></div></div>
        </div>
    </div>
    <div class="p-5 space-y-5">
        ...
        <?php if(data_get($checklist, 'behavior')): ?>
            <div class="rounded-xl border border-slate-200 overflow-hidden"><div class="bg-[#003829] px-4 py-3 text-white"><p class="text-xs font-bold uppercase">Kedisiplinan dan Komunikasi</p></div><div class="overflow-x-auto"><table class="min-w-[760px] w-full table-fixed text-xs"><colgroup><col class="w-14"><col class="w-16"><col><col class="w-14"><col class="w-14"><col class="w-72"></colgroup><thead class="bg-slate-50 text-slate-500 uppercase"><tr><th class="px-3 py-2 text-left">No</th><th class="px-3 py-2 text-left">Aspek</th><th class="px-3 py-2 text-left">Item Evaluasi</th><th class="px-3 py-2 text-center">K</th><th class="px-3 py-2 text-center">BK</th><th class="px-3 py-2 text-left">Catatan Penguji</th></tr></thead><tbody class="divide-y divide-slate-100"><?php $__currentLoopData = data_get($checklist, 'behavior', []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $itemIndex => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><tr class="align-top"><td class="px-3 py-3 font-bold"><?php echo e($item['code'] ?? ($itemIndex + 1)); ?></td><td class="px-3 py-3 text-slate-500"><?php echo e($item['kind'] ?? '-'); ?></td><td class="px-3 py-3 text-slate-700 leading-relaxed"><?php echo e($item['label'] ?? 'Item kedisiplinan'); ?></td><td class="px-3 py-3 text-center"><?php if(($item['status'] ?? null) === 'K'): ?><span class="text-[#00A859] font-black">✓</span><?php endif; ?></td><td class="px-3 py-3 text-center"><?php if(($item['status'] ?? null) === 'BK'): ?><span class="text-rose-600 font-black">✓</span><?php endif; ?></td><td class="px-3 py-3 text-slate-500 leading-relaxed break-words"><?php echo e($item['note'] ?? '-'); ?></td></tr><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></tbody></table></div></div>
        <?php endif; ?>
    </div>
</section>
