...
<?php $component->withAttributes([]); ?>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-7">
        <?php $__currentLoopData = [['Menunggu Review', $counts['submitted'], 'blue'], ['Sudah Diverifikasi', $counts['verified'], 'emerald']]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as [$label, $value, $color]): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="p-5 border-b border-slate-100 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div><h2 class="font-bold text-slate-800">Antrean Logbook</h2><p class="text-xs text-slate-500 mt-1">Prioritaskan pengajuan terbaru untuk diedit dan disetujui.</p></div>
            <form class="flex gap-2" method="GET"><input name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari NRP atau logbook..." class="text-xs rounded-xl border-slate-300 focus:border-[#00A859] focus:ring-[#00A859]"><select name="status" class="text-xs rounded-xl border-slate-300"><option value="all">Semua status</option><option value="submitted" <?php if(request('status') === 'submitted'): echo 'selected'; endif; ?>>Menunggu review</option><option value="verified" <?php if(request('status') === 'verified'): echo 'selected'; endif; ?>>Terverifikasi</option></select><button class="px-4 rounded-xl bg-[#003829] text-white text-xs font-bold">Filter</button></form>
        </div>
