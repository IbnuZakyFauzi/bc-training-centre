<section class="mt-8 bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden" x-data="{ action: '<?php echo e(old('action', 'verify')); ?>' }">
    ...
    <?php else: ?>
        <form method="POST" action="<?php echo e(route('trainer.reviews.evaluate', $logbook->id)); ?>" enctype="multipart/form-data" class="p-6 space-y-5">
            <?php echo csrf_field(); ?>
            ...
            <div class="grid gap-5 lg:grid-cols-2">
                ...
                <label class="block rounded-xl border border-amber-200 bg-amber-50 p-5 cursor-pointer">
                    ...
                    <textarea name="revision_instruction" rows="3" class="mt-2 w-full rounded-lg border-amber-300 text-xs" placeholder="Tuliskan bagian yang perlu diperbaiki..." :disabled="action !== 'revision'"><?php echo e(old('revision_instruction')); ?></textarea>
                </label>
            </div>
...
<?php /**PATH D:\KULIAH\BERAU COAL INTERN\logbook\resources\views\trainer\reviews\partials\decision-form.blade.php ENDPATH**/ ?>