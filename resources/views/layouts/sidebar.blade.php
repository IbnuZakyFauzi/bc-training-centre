    ...
    <div class="h-16 px-6 flex items-center justify-between border-b border-emerald-900/60 bg-[#002B1F]">
        <div class="flex items-center space-x-3">
            ...
            <div>
                ...
                <p class="text-[10px] text-emerald-300 font-medium tracking-wider uppercase mt-1">{{ $roleLabel }} Portal</p>
            </div>
        </div>
    </div>
        ...
        @if($currentSection)
            <div class="space-y-1">
                <a href="{{ route($dashboardRoute) }}"
                   class="flex items-center px-3 py-2.5 text-xs font-semibold rounded-lg transition-colors group {{ request()->routeIs($currentSection['items'][0]['match']) ? 'bg-[#00A859] text-white shadow-md' : 'text-emerald-100 hover:bg-emerald-900/60 hover:text-white' }}">
                    <svg class="w-4 h-4 mr-3 {{ request()->routeIs($currentSection['items'][0]['match']) ? 'text-white' : 'text-emerald-400 group-hover:text-white' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div>
                <div class="px-3 mb-2 text-[11px] font-bold text-emerald-400 uppercase tracking-widest flex items-center justify-between">
                    ...
                    <span class="bg-[#F5A623] text-gray-900 text-[9px] font-extrabold px-1.5 py-0.5 rounded">{{ $currentSection['tag'] }}</span>
                </div>

                <nav class="space-y-1">
                    @foreach($currentSection['items'] as $item)
                        @continue($item['route'] === $dashboardRoute)
                        <a href="{{ route($item['route']) }}"
                           class="flex items-center px-3 py-2.5 text-xs font-semibold rounded-lg transition-colors group {{ request()->routeIs($item['match']) ? 'bg-[#00A859] text-white shadow-md' : 'text-emerald-100 hover:bg-emerald-900/60 hover:text-white' }}">
                            <svg class="w-4 h-4 mr-3 {{ request()->routeIs($item['match']) ? 'text-white' : 'text-emerald-400 group-hover:text-white' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
