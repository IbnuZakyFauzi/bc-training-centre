...
<header class="h-16 bg-white border-b border-slate-200 px-6 flex items-center justify-between shadow-xs z-10">
    <div class="flex items-center space-x-4">
        ...
        <div class="flex items-center space-x-2 text-xs">
            ...
            <span class="text-slate-500 font-medium">{{ $roleLabel }} Dashboard</span>
        </div>
    </div>
    ...
    <div class="flex items-center space-x-4">
        <div class="hidden md:flex items-center space-x-2 px-3 py-1 bg-emerald-50 rounded-full border border-emerald-200">
            ...
            <span class="text-xs font-semibold text-[#003829]">{{ $roleLabel }} Active</span>
        </div>
        ...
        <div class="relative" x-data="{ open: false }">
            ...
            <div x-show="open" @click.away="open = false" x-cloak class="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-lg border border-slate-200 py-2 z-50">
                <div class="px-4 py-3 border-b border-slate-100 flex items-center justify-between gap-3">
                    <div>
                        <p class="text-xs font-bold text-slate-800">{{ $user->name ?? 'User' }}</p>
                        <p class="text-[11px] text-slate-500 mt-0.5">{{ $user->nrp ?? '-' }} · {{ $roleLabel }}</p>
                    </div>
                    <div class="w-10 h-10 rounded-full bg-[#003829] text-white flex items-center justify-center font-bold text-xs">
                        {{ collect(explode(' ', $user->name ?? 'U'))->filter()->take(2)->map(fn ($part) => strtoupper(substr($part, 0, 1)))->implode('') ?: 'U' }}
                    </div>
                </div>
                <a href="{{ route($quickAction['route']) }}" class="px-4 py-3 block hover:bg-emerald-50/60 transition">
                    <p class="text-xs font-semibold text-[#003829]">Akses Cepat</p>
                    <p class="text-[11px] text-slate-500 mt-0.5">{{ $quickAction['label'] }}</p>
                </a>
                <a href="{{ route('profile.edit') }}" class="px-4 py-3 block hover:bg-emerald-50/60 transition">
        ...
        <div class="flex items-center space-x-3 pl-3 border-l border-slate-200">
            <img class="w-8 h-8 rounded-full border-2 border-[#00A859] object-cover" src="https://ui-avatars.com/api/?name={{ urlencode($user->name ?? 'User') }}&background=003829&color=fff" alt="User Avatar">
            <div class="hidden lg:block text-left">
                <span class="text-xs font-bold text-slate-800 block leading-tight">{{ $user->name ?? 'User' }}</span>
                <span class="text-[10px] text-slate-500 font-medium block">{{ $user->nrp ?? '-' }} · {{ $roleLabel }}</span>
            </div>
        </div>
    </div>
</header>
