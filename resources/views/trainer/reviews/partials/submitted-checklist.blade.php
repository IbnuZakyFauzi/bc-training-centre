...
<section class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
    ...
    <div class="mx-5 mt-5 rounded-xl border border-slate-300 overflow-hidden text-[11px]">
        <div class="grid grid-cols-1 md:grid-cols-2">
            ...
            <div class="divide-y divide-slate-200"><div class="grid grid-cols-[130px_1fr]"><span class="p-2 font-bold bg-slate-50">PERUSAHAAN</span><span class="p-2">{{ data_get($payload, 'meta.company', 'PT BERAU COAL / PT MTL') }}</span></div><div class="grid grid-cols-[130px_1fr]"><span class="p-2 font-bold bg-slate-50">TIPE ALAT</span><span class="p-2">{{ $logbook->equipmentCategory->name ?? '-' }} ({{ $categoryCode }})</span></div><div class="grid grid-cols-[130px_1fr]"><span class="p-2 font-bold bg-slate-50">NO ALAT</span><span class="p-2">{{ $logbook->equipment->unit_code ?? '-' }}</span></div><div class="grid grid-cols-[130px_1fr]"><span class="p-2 font-bold bg-slate-50">HM / KM</span><span class="p-2">{{ number_format($logbook->hm_start, 1) }} → {{ number_format($logbook->hm_end, 1) }}</span></div></div>
        </div>
    </div>
    <div class="p-5 space-y-5">
        ...
        @if(data_get($checklist, 'behavior'))
            <div class="rounded-xl border border-slate-200 overflow-hidden"><div class="bg-[#003829] px-4 py-3 text-white"><p class="text-xs font-bold uppercase">Kedisiplinan dan Komunikasi</p></div><div class="overflow-x-auto"><table class="min-w-[760px] w-full table-fixed text-xs"><colgroup><col class="w-14"><col class="w-16"><col><col class="w-28"><col class="w-72"></colgroup><thead class="bg-slate-50 text-slate-500 uppercase"><tr><th class="px-3 py-2 text-left">No</th><th class="px-3 py-2 text-left">Aspek</th><th class="px-3 py-2 text-left">Item Evaluasi</th><th class="px-3 py-2 text-center">Status</th><th class="px-3 py-2 text-left">Catatan Penguji</th></tr></thead><tbody class="divide-y divide-slate-100">@foreach(data_get($checklist, 'behavior', []) as $itemIndex => $item)<tr class="align-top"><td class="px-3 py-3 font-bold">{{ $item['code'] ?? ($itemIndex + 1) }}</td><td class="px-3 py-3 text-slate-500">{{ $item['kind'] ?? '-' }}</td><td class="px-3 py-3 text-slate-700 leading-relaxed">{{ $item['label'] ?? 'Item kedisiplinan' }}</td><td class="px-3 py-3 text-center font-bold">{{ $item['status'] ?? '-' }}</td><td class="px-3 py-3 text-slate-500 leading-relaxed break-words">{{ $item['note'] ?? '-' }}</td></tr>@endforeach</tbody></table></div></div>
        @endif
    </div>
</section>
