<x-app-layout>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-7">
        @foreach([['Menunggu Review', $counts['submitted'], 'blue'], ['Sudah Diverifikasi', $counts['verified'], 'emerald']] as [$label, $value, $color])
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="p-5 border-b border-slate-100 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <form class="flex gap-2" method="GET"><input name="search" value="{{ request('search') }}" placeholder="Cari NRP atau logbook..." class="text-xs rounded-xl border-slate-300 focus:border-[#00A859] focus:ring-[#00A859]"><select name="status" class="text-xs rounded-xl border-slate-300"><option value="all">Semua status</option><option value="submitted" @selected(request('status') === 'submitted')>Menunggu review</option><option value="verified" @selected(request('status') === 'verified')>Terverifikasi</option></select><button class="px-4 rounded-xl bg-[#003829] text-white text-xs font-bold">Filter</button></form>
        </div>
