...
<form action="{{ $isTrainerEditing ? route('trainer.reviews.update', $logbook->id) : ($isEditing ? route('ojt.logbooks.update', $logbook->id) : route('ojt.logbooks.store')) }}" method="POST" enctype="multipart/form-data"
      x-data="{
          categoryId: '{{ old('equipment_category_id', $selectedCategoryId) }}',
          equipmentId: '{{ old('equipment_id', $isEditing ? $logbook->equipment_id : '') }}',
          categoryMap: @js($categoryMap),
          equipmentMap: @js($equipmentMap),
          company: @js(old('sop_payload.meta.company', data_get($formPayload, 'meta.company', 'PT BERAU COAL / PT MTL'))),
          ...
          assessmentMode: @js(old('sop_payload.meta.assessment_mode', data_get($formPayload, 'meta.assessment_mode', 'pendampingan'))),
          assessmentStage: @js(old('sop_payload.meta.assessment_stage', data_get($formPayload, 'meta.assessment_stage', 'bulanan'))),
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2">
            <div class="border-b border-slate-900 lg:border-b-0 lg:border-r">
                <div class="grid grid-cols-[120px_minmax(0,1fr)] gap-x-3 gap-y-0 text-[11px] text-slate-900">
                    ...
                    <div class="px-3 py-1.5 border-b border-slate-900">
                        <input type="date" name="date" value="{{ old('date', $isEditing ? optional($logbook->date)->format('Y-m-d') : date('Y-m-d')) }}" required class="w-full border-0 bg-transparent p-0 text-[11px] font-medium focus:ring-0">
                    </div>
                    ...
                    <div class="px-3 py-1.5 border-b border-slate-900">
                        <select name="shift" required class="w-full border-0 bg-transparent p-0 text-[11px] font-medium focus:ring-0">
                            <option value="day" {{ old('shift', $isEditing ? $logbook->shift : '') == 'day' ? 'selected' : '' }}>Shift 1 (Siang: 07.00 - 17.00)</option>
                            <option value="night" {{ old('shift', $isEditing ? $logbook->shift : '') == 'night' ? 'selected' : '' }}>Shift 2 (Malam: 19.00 - 05.00)</option>
                        </select>
                    </div>

                    <div class="px-3 py-2 font-semibold border-b border-slate-900">LOKASI (OJT)</div>
                    <div class="px-3 py-1.5 border-b border-slate-900">
                        <input type="text" name="location" value="{{ old('location', $isEditing ? $logbook->location : '') }}" placeholder="Contoh: Pit H1 East - Bench 45" required class="w-full border-0 bg-transparent p-0 text-[11px] font-medium focus:ring-0">
                    </div>
            ...
            <div>
                <div class="grid grid-cols-[120px_minmax(0,1fr)] gap-x-3 gap-y-0 text-[11px] text-slate-900">
                    ...
                    <div class="px-3 py-1.5 border-b border-slate-900">
                        <input type="text" name="equipment_number" value="{{ old('equipment_number', $isEditing ? $logbook->equipment_number : '') }}" placeholder="Ketik nomor alat, contoh: DZ-123" required class="w-full border-0 bg-transparent p-0 text-[11px] font-medium focus:ring-0">
                    </div>
            ...
            <div>
                ...
                <select name="department_id" required class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-[#00A859] focus:bg-white transition">
                    @foreach($departments as $dept)
                        <option value="{{ $dept->id }}" {{ old('department_id', $isEditing ? $logbook->department_id : $user->department_id) == $dept->id ? 'selected' : '' }}>{{ $dept->code }} - {{ $dept->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <select name="trainer_id" required class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-[#00A859] focus:bg-white transition">
                    ...
                    @foreach($trainers as $tr)
                        <option value="{{ $tr->id }}" {{ old('trainer_id', $isEditing ? $logbook->trainer_id : '') == $tr->id ? 'selected' : '' }}>{{ $tr->name }} ({{ $tr->nrp }})</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <select name="supervisor_id" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-[#00A859] focus:bg-white transition">
                    ...
                    @foreach($supervisors as $spv)
                        <option value="{{ $spv->id }}" {{ old('supervisor_id', $isEditing ? $logbook->supervisor_id : '') == $spv->id ? 'selected' : '' }}>{{ $spv->name }} ({{ $spv->nrp }})</option>
                    @endforeach
                </select>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-6 items-end">
            <div>
                ...
                <input type="time" name="start_time" value="{{ old('start_time', $isEditing ? $logbook->start_time : '07:00') }}" required class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-[#00A859] focus:bg-white transition">
            </div>
            <div>
                ...
                <input type="time" name="finish_time" value="{{ old('finish_time', $isEditing ? $logbook->finish_time : '17:00') }}" required class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-[#00A859] focus:bg-white transition">
            </div>
            <div>
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-200 py-4 px-6 md:px-8 z-40 flex items-center justify-between shadow-2xl">
        ...
        <div class="flex items-center space-x-3 w-full lg:w-auto justify-end">
            @unless($isTrainerEditing)
                <button type="submit" name="action_type" value="draft" formnovalidate class="px-6 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl shadow-xs transition">{{ $isEditing ? 'Simpan Perubahan Draft' : 'Save Draft' }}</button>
            @endunless
            <button type="submit" name="action_type" value="submit" class="px-7 py-2.5 bg-[#00A859] hover:bg-emerald-600 text-white font-bold text-xs rounded-xl shadow-md transition transform hover:-translate-y-0.5 flex items-center">
                ...
                {{ $isTrainerEditing ? 'Simpan Perubahan' : ($isEditing ? 'Kirim Ulang ke Trainer' : 'Submit Logbook') }}
            </button>
        </div>
    </div>
</form>
