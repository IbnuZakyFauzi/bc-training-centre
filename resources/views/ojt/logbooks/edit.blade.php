...
<x-app-layout>
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <div class="flex items-center space-x-2 text-xs font-semibold text-[#00A859] mb-1">
                ...
                <span>/</span>
                <span class="text-slate-500">Edit Logbook ({{ $logbook->logbook_number }})</span>
            </div>
            <div class="flex items-center space-x-3">
                <h1 class="text-xl font-extrabold text-slate-800 tracking-tight">Edit Logbook {{ $logbook->logbook_number }}</h1>
                <x-badge :status="$logbook->status" />
            </div>
        </div>
        <a href="{{ route('ojt.logbooks.show', $logbook->id) }}" class="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-xs font-bold transition">
            Batal
        </a>
    </div>
    ...
    <form action="{{ route('ojt.logbooks.update', $logbook->id) }}" method="POST" enctype="multipart/form-data" 
          x-data="{ 
              categoryId: '{{ old('equipment_category_id', $logbook->equipment_category_id) }}',
              equipmentId: '{{ old('equipment_id', $logbook->equipment_id) }}',
              equipmentMap: @js($equipmentMap),
              hmStart: {{ old('hm_start', $logbook->hm_start) }}, 
              hmEnd: {{ old('hm_end', $logbook->hm_end) }}, 
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="date" value="{{ old('date', $logbook->date->format('Y-m-d')) }}" required class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-[#00A859] focus:bg-white transition">
                </div>
                ...
                <div>
                    ...
                    <select name="shift" required class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-[#00A859] focus:bg-white transition">
                        <option value="day" {{ old('shift', $logbook->shift) == 'day' ? 'selected' : '' }}>Shift 1 (Siang: 07.00 - 17.00)</option>
                        <option value="night" {{ old('shift', $logbook->shift) == 'night' ? 'selected' : '' }}>Shift 2 (Malam: 19.00 - 05.00)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="department_id" required class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-[#00A859] focus:bg-white transition">
                        @foreach($departments as $dept)
                            <option value="{{ $dept->id }}" {{ old('department_id', $logbook->department_id) == $dept->id ? 'selected' : '' }}>
                                {{ $dept->code }} - {{ $dept->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="location" value="{{ old('location', $logbook->location) }}" required class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-[#00A859] focus:bg-white transition">
                </div>
                ...
                <div>
                    ...
                    <select name="equipment_category_id" x-model="categoryId" @change="if (!filteredEquipments.some(item => String(item.id) === String(equipmentId))) equipmentId = ''" required class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-[#00A859] focus:bg-white transition">
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}" {{ old('equipment_category_id', $logbook->equipment_category_id) == $cat->id ? 'selected' : '' }}>
                                {{ $cat->name }} ({{ $cat->code }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="trainer_id" required class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-[#00A859] focus:bg-white transition">
                        @foreach($trainers as $tr)
                            <option value="{{ $tr->id }}" {{ old('trainer_id', $logbook->trainer_id) == $tr->id ? 'selected' : '' }}>
                                {{ $tr->name }} ({{ $tr->nrp }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="supervisor_id" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-[#00A859] focus:bg-white transition">
                        ...
                        @foreach($supervisors as $spv)
                            <option value="{{ $spv->id }}" {{ old('supervisor_id', $logbook->supervisor_id) == $spv->id ? 'selected' : '' }}>
                                {{ $spv->name }} ({{ $spv->nrp }})
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div class="p-6">
                ...
                <textarea name="daily_activity" rows="6" required class="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl text-xs font-mono leading-relaxed focus:ring-2 focus:ring-[#00A859] focus:bg-white transition">{{ old('daily_activity', $logbook->daily_activity) }}</textarea>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 items-end">
                <div>
                    ...
                    <input type="time" name="start_time" value="{{ old('start_time', $logbook->start_time) }}" required class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-[#00A859] focus:bg-white transition">
                </div>
                ...
                <div>
                    ...
                    <input type="time" name="finish_time" value="{{ old('finish_time', $logbook->finish_time) }}" required class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-[#00A859] focus:bg-white transition">
                </div>
        ...
        <div class="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-200 py-4 px-8 z-40 flex items-center justify-between shadow-2xl">
            <a href="{{ route('ojt.logbooks.show', $logbook->id) }}" class="text-xs font-bold text-slate-500 hover:text-slate-700">Batal</a>
